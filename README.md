...
Note that the model below shows a browser implying a JavaScript client.  This is one model of possible access patterns.  There is nothing precluding a service or tooling written in any language from using the APIs directly, foregoing this JavaScript client library.

![components](docs/.images/provided.png)

* a user interfaces with a configured blockchain external to *overhide*
* a user interfaces with an in browser "wallet" extension unrelated to app (service)
* the "wallet" implements the JavaScript *overhide* client code making it available to any in-browser app, service, or tool
...
* client-api/wallet secures user's data client side--secured before leaves the browser
...
## [JavaScript Client Library](docs/.generated/client.md)

This [JavaScript client library](docs/.generated/client.md) is a wrapper for the above APIs as generated by Swagger.  The library benefits app/service developers in the standard ways.

By convention this library is to be provided in the global context via an `overhide` object.

An instance of this object should be made available to app/service code running in a browser via a "wallet" browser extension.  Think `web3` as provided by *MetaMask* in *Chrome*.  This setup effectively removes the need to trust every app/service developer; just the wallet needs to be trusted.
